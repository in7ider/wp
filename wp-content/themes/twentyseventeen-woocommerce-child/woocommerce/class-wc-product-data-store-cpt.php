...
class WC_Product_Data_Store_CPT extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface, WC_Product_Data_Store_Interface {
	...
	*/
		...
		$id = wp_insert_post(
			apply_filters(
				'woocommerce_new_product_data', array(
					...
					'post_title'     => $product->get_name() ? $product->get_name() : __( 'movies', 'woocommerce' ),
	...
	public function read( &$product ) {
		...
		if ( ! $product->get_id() || ! $post_object || 'movies' !== $post_object->post_type ) {
	...
	public function delete( &$product, $args = array() ) {
		$id        = $product->get_id();
		$post_type = $product->is_type( 'variation' ) ? 'product_variation' : 'movies';
	...
	*/
	...
	public function get_on_sale_products() {
		...
		return $wpdb->get_results(
			...
			$wpdb->prepare(
				...
				$outofstock_join
				WHERE post.post_type IN ( 'movies', 'product_variation' )
	...
	public function get_featured_product_ids() {
		...
		return get_posts(
			array(
				'post_type'      => array( 'movies', 'product_variation' ),
				'posts_per_page' => -1,
				'post_status'    => 'publish',
	...
	public function is_existing_sku( $product_id, $sku ) {
		...
		return $wpdb->get_var(
			$wpdb->prepare(
				...
				WHERE $wpdb->posts.post_type IN ( 'movies', 'product_variation' )
	...
	public function get_product_id_by_sku( $sku ) {
		...
		$id = $wpdb->get_var(
			$wpdb->prepare(
				...
				WHERE posts.post_type IN ( 'movies', 'product_variation' )
	...
	public function search_products( $term, $type = '', $include_variations = false, $all_statuses = false ) {
		...
		$post_types    = $include_variations ? array( 'movies', 'product_variation' ) : array( 'movies' );
	...
	protected function get_wp_query_args( $query_vars ) {
		...
		if ( 'variation' === $query_vars['type'] ) {
			...
			$wp_query_args['post_type']   = array( 'product_variation', 'movies' );
			...
			$wp_query_args['post_type']   = 'movies';
			$wp_query_args['tax_query'][] = array(
				'taxonomy' => 'product_type',
				'field'    => 'slug',
				'terms'    => $query_vars['type'],
	...
	public function query( $query_vars ) {
		...
		if ( isset( $query_vars['return'] ) && 'objects' === $query_vars['return'] && ! empty( $query->posts ) ) {
			...
			update_post_caches( $query->posts, array( 'movies', 'product_variation' ) );
		}
